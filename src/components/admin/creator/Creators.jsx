...
const Creators = () => {
    ...
    const {searchData,setCreators,creators} = useGlobalStore(state=>state);
    const filterCreators = ()=>{
        const filteredItems = creators.filter(creator=>
            creator.name.toLocaleLowerCase().includes(searchData)||
            creator.email.toLocaleLowerCase().includes(searchData)||
            creator.status.toLocaleLowerCase().includes(searchData)
        )
            if(searchData == ''){
                ...
                creators.map(creator=>{
                    return(
                        <li className="relative">
                            <div className={`${creator.status=='approved'&&'border-y-[3px] md:border-y-0 md:border-x-[3px] border-green-500'} ${creator.status=='banned'&&'border-y-[3px] md:border-y-0 md:border-x-[3px] border-red-500'} ${creator.status=='suspended'&&'border-y-[3px] md:border-y-0 md:border-x-[3px] border-black'} flex flex-col gap-4 md:gap-0 md:flex-row justify-between items-center mt-5 shadow-md capitalize p-5 rounded-md relative`}>
                               ...
                               <p>{creator.name}</p>
                                ...
                                <p className='basis-[15%]'>{creator.gender}</p>
                                </div>
                                <div className='flex justify-between  w-full md:w-auto md:basis-[15%]'>
                                <p className='font-[700] capitalize md:hidden'>status:</p>
                                <p className={`basis-[15%] ${creator.status=='approved'&&' md:border-y-0  text-green-500'} ${creator.status=='banned'&&' md:border-y-0  text-red-500'} ${creator.status=='suspended'&&' md:border-y-0  text-black'} `}>{creator.status}</p>
                                </div>
                                <p className='basis-[15%] ' ><MoreHorizontal className='cursor-pointer'
                                 onClick={()=>handleOptions(creator.id)}/> </p>
                            </div>

                            {id === creator.id&& <div className='shadow-lg flex gap-3  py-5 z-30 px-4 bg-white rounded-md w-[max-content] absolute top-6 right-0'>
